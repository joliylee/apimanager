...
<header>
	<div class="container navi-breadcrumb">
		<ul class="breadcrumb">
        	...
        	<li><a class="active" href="<?php echo U('project/create');?>">新建工程</a></li>
...
<!-- body container -->
<div class="container">
	<form action="<?php echo U('project/create');?>" method="POST">
		<!-- basic information	 -->
		<legend>基本信息</legend>
		...
		<select name="status" id="">
			<?php if(is_array($projectStatus)): foreach($projectStatus as $k=>$vo): ?><option value="<?php echo ($k); ?>"><?php echo ($vo); ?></option><?php endforeach; endif; ?>
		</select>

		<!-- server path -->
		<legend>服务器地址</legend>
		<label for="">正式服务器</label>
		<input type="text" name="host_product" class="input-xlarge" placeholder="选填" >
		<label for="">测试服务器</label>
		<input type="text" name="host_develop" class="input-xlarge" placeholder="选填">
		<label for="">伪数据服务器</label>
		<input type="text" name="host_faker" class="input-xlarge" placeholder="选填">
		<p><i class="icon-exclamation-sign"></i>必须以http://开头, 可以包含API路径</p>

		

		<legend>项目说明</legend>
		<textarea name="detail" id="" rows="20"></textarea>
